...
class SigmoidCurve_numba:
    ...
    def __init__(self, numSigmoids, dMin=0.5, dMax=6.):
        ...
        self.Imax = np.zeros(self.sNum)
        self.Imin = np.zeros(self.sNum)   
        
        self.sAmps = np.zeros(self.sNum)
        self.max_sAmps = np.zeros(self.sNum) # need to init because hasattr 
        self.min_sAmps = np.zeros(self.sNum) # is incompatible with jitclass 
    ...
    def copy(self):
        """Takes the place of copy.copy(), which does not work with jitclass.
        """
        
        newCopy = SigmoidCurve_numba(self.sNum,self.dMin,self.dMax)
        newCopy.I_of_r = self.I_of_r.copy()
        newCopy.sAmps = self.sAmps.copy()
        newCopy._sWidth = self._sWidth
        newCopy._sStart = self._sStart.copy()
        newCopy.Ix = self.Ix.copy()
        
        newCopy.Imax = self.Imax.copy()
        newCopy.Imin = self.Imin.copy()
        newCopy.max_sAmps = self.max_sAmps.copy()
        newCopy.min_sAmps = self.min_sAmps.copy()
...
def sigmoid_to_numba(theCurve):
    """Converts a SigmoidCurve object to SigmoidCurve_numba for rapid access
    """
    
    newCurve = SigmoidCurve_numba(3,2,1) #arbitrary init values
    
    for memberName in spec:
        if hasattr(theCurve, memberName[0]):
            theCurveVal=getattr(theCurve,memberName[0])
            setattr(newCurve,memberName[0],theCurveVal)
    
    return newCurve    


def decant_UM_terms(theUM):
    """Extracts curves from the universal model, so that curve values can be 
    rapidly interpolated and accessed.
    """
        
    termCurves=[]
    for term in theUM.termsList:
        if term.curve != None:
            termCurves +=[sigmoid_to_numba(term.curve)] #output the numba sigmoidCurve
        else:
            termCurves += [[term.val, term.max, term.min]]
        

    return termCurves

def readTerms(termsList):
    """Extracts key val/max/min numbers from the termCurves. Output format is
    termNums[term][2D numpy array of val/max/min/isCurve].  These 'val' values are the 
    input to the discriminator model, and will be saved to a long 'X' list.
    """
    termNums=[]
    for term in termsList:
        if type(term) != list: # if it's a curve
            termNums += [np.asarray([term.I_of_r,term.Imax,term.Imin]).T]
        else:
            termNums += [np.asarray([term])]
