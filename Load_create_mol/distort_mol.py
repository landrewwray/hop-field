# -*- coding: utf-8 -*-
"""
Created on Tue Jun  7 17:56:08 2022
...
def distort_bond(theCoords, _theBonds_, bondNum, distortDist=0.01, distortAxis=[]):
    ...
    _theCoords_ = copy.deepcopy(
        theCoords
    )  # modified data will be returned in newCoords
    if distortAxis == []:
        distortAxis = bondNum

    isBuckle = 0
    chosenBondVector = (
        _theCoords_[_theBonds_[bondNum, 2], :] - _theCoords_[_theBonds_[bondNum, 1], :]
    )
    # if distortAxis is an integer, indicating a bond axis
    if type(distortAxis) == int:
        distortVector = (
            _theCoords_[_theBonds_[distortAxis, 2], :]
            - _theCoords_[_theBonds_[distortAxis, 1], :]
        )
    ...
    if isBuckle:
        distortVector = np.cross(
            np.cross(chosenBondVector, distortVector), chosenBondVector
        )
...
def distort_molecule(
    ...
    newCoords, distortVector = distort_bond(
        theCoords, theBonds, bondNum, distortDist, distortAxis
    )
    ...
    while len(startingPointSet) > 0:
        ...
        for startingPoint in startingPointSet:
            connectedPoints = connectedPoints.union(
                theBonds[np.where(theBonds[:, 1] == startingPoint), 2].flatten()
            )
            connectedPoints = connectedPoints.union(
                theBonds[np.where(theBonds[:, 2] == startingPoint), 1].flatten()
            )
...
def distortionList(
    theCoords, theBonds, bondNumArray, fixPosSet=set(), distortDist=0.01, distortAxis=[]
):
    newCoordsList = [
        distort_molecule(
            theCoords,
            theBonds,
            i,
            bondNumArray[np.arange(len(bondNumArray)) != i],
            distortDist=0.01,
            distortAxis=[],
        )
        for i in bondNumArray
    ]
