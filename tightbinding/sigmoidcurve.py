...
class TestClass:
    ...
    @theList.setter
    def theList(self, newList):
        self._theList=newList
...
class SigmoidCurve:
    ...
    Note that updating the curve amplitudes (self.I_of_r) will automatically update 
    the sigmoid amplitudes (sAmp), but one must run self.__make_I_of_r__ to update 
    self.I_of_r after changing self.sAmp.  This will happen automatically if 
    _maintain_I_of_r==True, which is the current default.
